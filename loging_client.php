<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if(empty($username_err) && empty($password_err)){
			...
			$sql = "SELECT user_id, user_name, user_pass , user_phone,user_email, user_address FROM user WHERE user_name = ?";
			...
			if($stmt = $conn->prepare($sql)){
				...
				if($stmt->execute()){
					...
					if($stmt->num_rows == 1){                    
						...
						$stmt->bind_result($id, $username, $hashed_password,$user_phone,$user_email,$user_address);
